--- custom version @ 2009年 11月 30日 星期一 16:29:01 CST
-- add ansii terminal color 
local ansi_color_reset = '\027[0m'
local ansi_color_test_class = '\027[35;1m'  -- 35	Magenta
local ansi_color_test_proc = '\027[34m'   -- 34 blue

local ansi_color_fail = '\027[31;4m'   -- 31 Red
local ansi_color_success = '\027[34m'

local ansi_color_separator = '\027[41;1m'

-- then make screen scrool
local print_orig = print
local write_orig = io.write
...
UnitResult = { -- class
	...
	function UnitResult:displayClassName()
		print( '>>>>>>>' .. ansi_color_test_class .. '>> '.. self.currentClassName .. ansi_color_reset )
	end

	function UnitResult:displayTestName()
		if self.verbosity > 0 then
			print( ">>> ".. ansi_color_test_proc .. self.currentTestName .. ansi_color_reset )
		end
	end
	...
	function UnitResult:displayFailure( errorMsg )
		if self.verbosity == 0 then
			...
			print( errorMsg )
			print( ansi_color_fail, 'Failed' , ansi_color_reset)
		end
	end

	function UnitResult:displaySuccess()
		if self.verbosity > 0 then
			--print (ansi_color_success, "Ok" , ansi_color_reset)
		else 
			io.stdout:write(".")
		end
	end
	...
	function UnitResult:displayFinalResult()
		local begin_ascii = UnitResult.failureCount == 0 
			and ansi_color_success or  ansi_color_separator
		print( begin_ascii .. "=========================================================" .. ansi_color_reset)
